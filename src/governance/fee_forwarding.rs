...
impl FeeForwardingTracker {
    ...
    pub fn from_network_string(
        ...
        Self::new(
            pool.clone(),
            ContributionTracker::new(pool),
            commons_addresses,
            Self::network_from_string(network_str),
        )
    }

    /// Create from network string with auto-registration enabled
    pub fn from_network_string_with_auto_registration(
        pool: SqlitePool,
        contribution_tracker: ContributionTracker,
        commons_addresses: Vec<String>,
        network_str: &str,
    ...
    fn network_from_string(network_str: &str) -> bitcoin::Network {
    ...
    /// Uses blvm-consensus's calculate_tx_id which properly serializes and hashes transactions
    ...
    pub(crate) fn calculate_tx_hash(&self, tx: &blvm_protocol::Transaction) -> String {
        self.calculate_tx_hash_internal(tx)
    }
    ...
    fn calculate_tx_hash_internal(&self, tx: &blvm_protocol::Transaction) -> String {
        use blvm_protocol::block::calculate_tx_id;

        // Use the proper transaction ID calculation from blvm-consensus
