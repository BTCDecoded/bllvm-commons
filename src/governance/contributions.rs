...
use crate::economic_nodes::auto_registration::CommonsContributorAutoRegistrar;
use crate::economic_nodes::registry::EconomicNodeRegistry;

/// Contribution tracking service
pub struct ContributionTracker {
    pool: SqlitePool,
    /// Optional auto-registrar for Commons Contributors
    auto_registrar: Option<Arc<CommonsContributorAutoRegistrar>>,
}
...
impl ContributionTracker {
    ...
    /// Create a new contribution tracker with auto-registration enabled
    pub fn with_auto_registration(
        pool: SqlitePool,
        registry: Arc<EconomicNodeRegistry>,
        zap_tracker: Option<Arc<crate::nostr::zap_tracker::ZapTracker>>,
    ) -> Self {
        let auto_registrar = Arc::new(CommonsContributorAutoRegistrar::new(
            pool.clone(),
            registry,
            zap_tracker,
        ));
        Self {
            pool,
            auto_registrar: Some(auto_registrar),
        }
    }

    /// Record a merge mining contribution (1% of secondary chain rewards)
    /// Returns the contribution amount for potential auto-registration
    pub async fn record_merge_mining_contribution(
        ...
        // Auto-register if qualified (non-blocking, log errors but don't fail)
        if let Some(ref registrar) = self.auto_registrar {
            if let Err(e) = registrar.check_and_register(contributor_id, "merge_miner").await {
                warn!("Auto-registration check failed for merge miner {}: {}", contributor_id, e);
            }
        }
    ...
    pub async fn record_fee_forwarding_contribution(
        ...
        // Auto-register if qualified (non-blocking, log errors but don't fail)
        if let Some(ref registrar) = self.auto_registrar {
            if let Err(e) = registrar.check_and_register(contributor_id, "fee_forwarder").await {
                warn!("Auto-registration check failed for fee forwarder {}: {}", contributor_id, e);
            }
        }
    ...
    pub async fn record_zap_contribution(
        ...
        // Auto-register if qualified (non-blocking, log errors but don't fail)
        if let Some(ref registrar) = self.auto_registrar {
            if let Err(e) = registrar.check_and_register(contributor_id, "zap_user").await {
                warn!("Auto-registration check failed for zap user {}: {}", contributor_id, e);
            }
        }
