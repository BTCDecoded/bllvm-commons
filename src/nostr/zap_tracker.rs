...
pub struct ZapTracker {
    ...
    contribution_tracker: Option<Arc<crate::governance::ContributionTracker>>, // Optional tracker with auto-registration
}
...
impl ZapTracker {
    ...
    /// Create a new zap tracker with auto-registration enabled
    pub fn with_auto_registration(
        pool: SqlitePool,
        nostr_client: Arc<NostrClient>,
        bot_pubkeys: Vec<String>,
        contribution_tracker: Arc<crate::governance::ContributionTracker>,
    ) -> Self {
        Self {
            pool,
            nostr_client,
            bot_pubkeys,
            contribution_tracker: Some(contribution_tracker),
        }
    }
    ...
    /// Process a zap event and record it in the database (legacy method, uses new tracker)
    async fn process_zap(pool: &SqlitePool, recipient_pubkey: &str, zap: ZapEvent) -> Result<()> {
        Self::process_zap_with_tracker(pool, recipient_pubkey, zap, None).await
    }

    /// Process a zap event and record it in the database (with optional contribution tracker)
    async fn process_zap_with_tracker(
        pool: &SqlitePool,
        recipient_pubkey: &str,
        zap: ZapEvent,
        contribution_tracker: Option<Arc<crate::governance::ContributionTracker>>,
    ) -> Result<()> {
        ...
        if let Some(ref sender_pubkey) = zap.sender_pubkey {
            // Use provided tracker if available (with auto-registration), otherwise create new one
            let tracker = if let Some(ref ct) = contribution_tracker {
                ct.clone()
            } else {
                Arc::new(ContributionTracker::new(pool.clone()))
            };
            ...
            // Note: Auto-registration will be triggered by ContributionTracker if enabled
        }
