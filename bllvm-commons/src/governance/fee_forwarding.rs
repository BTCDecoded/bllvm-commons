...
impl FeeForwardingTracker {
    ...
    pub fn new(pool: SqlitePool, commons_addresses: Vec<String>, network: bitcoin::Network) -> Self {
        Self {
            pool: pool.clone(),
    ...
    pub fn from_network_string(pool: SqlitePool, commons_addresses: Vec<String>, network_str: &str) -> Self {
                ...
                if let Some(address) = address {
                    if self.commons_addresses.contains(&address) {
                        ...
                        let final_contributor_id = if let Some(id) = contributor_id {
                            ...
                            self.lookup_contributor_from_address(&address).await
