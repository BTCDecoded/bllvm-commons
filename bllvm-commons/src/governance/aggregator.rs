...
impl ContributionAggregator {
    ...
    pub async fn aggregate_merge_mining_monthly(
        &self,
        contributor_id: &str,
    ) -> Result<f64> {
        let now = Utc::now();
    ...
    pub async fn aggregate_fee_forwarding_monthly(
        &self,
        contributor_id: &str,
    ) -> Result<f64> {
        let now = Utc::now();
    ...
    pub async fn aggregate_zaps_cumulative(
        &self,
        contributor_id: &str,
    ) -> Result<f64> {
        let total: Option<f64> = sqlx::query_scalar(
    ...
    pub async fn update_all_weights(&self) -> Result<()> {
        ...
        self.weight_calculator.update_participation_weights().await?;
    ...
    pub async fn get_contributor_aggregates(
        ...
        let fee_forwarding = self.aggregate_fee_forwarding_monthly(contributor_id).await?;
        ...
        let participation_weight = self.weight_calculator
