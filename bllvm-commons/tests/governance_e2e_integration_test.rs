...
async fn test_complete_governance_flow_zap_veto_blocked() {
    ...
    assert!(result.veto_votes > result.support_votes, "Veto votes should exceed support");
}
...
async fn test_complete_governance_flow_weight_cap_enforcement() {
    ...
    // Step 4: Get total system weight
    ...
    assert!(whale_weight <= max_allowed + 0.01, "Whale weight must be capped at 5%");
    
    // Step 7: Verify whale cannot dominate
    let whale_percentage = whale_weight / total_weight;
    assert!(whale_percentage <= 0.05 + 0.0001, "Whale cannot exceed 5% of total");
}
...
async fn test_complete_governance_flow_combined_veto_systems() {
    ...
    // Combined: Neither threshold met individually, but together = 57% → should NOT block (each system independent)
