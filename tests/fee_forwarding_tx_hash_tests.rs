...
use blvm_protocol::{Block, OutPoint, Transaction, TransactionInput, TransactionOutput};
...
fn create_test_transaction() -> Transaction {
    Transaction {
        version: 2,
        inputs: SmallVec::from_vec(vec![TransactionInput {
            prevout: OutPoint {
                hash: [0u8; 32],
                index: 0,
            },
        ...
        outputs: SmallVec::from_vec(vec![TransactionOutput {
...
async fn test_calculate_tx_hash_consistency() {
    ...
    let tracker = FeeForwardingTracker::new(pool, vec![], Network::Bitcoin);
...
async fn test_fee_forwarding_duplicate_detection() {
    ...
    assert_eq!(
        contributions2.len(),
        0,
        "Duplicate transaction should not be recorded again"
    );
}
