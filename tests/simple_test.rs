...
async fn test_threshold_validation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Test tier-specific thresholds (using static methods for backward compatibility)
    let (required, total) = ThresholdValidator::get_tier_threshold_static(1);
    assert_eq!(required, 3);
    assert_eq!(total, 5);
    println!("✅ Tier 1 threshold: {}/{}", required, total);

    let (required, total) = ThresholdValidator::get_tier_threshold_static(2);
    assert_eq!(required, 4);
    assert_eq!(total, 5);
    println!("✅ Tier 2 threshold: {}/{}", required, total);

    let (required, total) = ThresholdValidator::get_tier_threshold_static(3);
    assert_eq!(required, 5);
    assert_eq!(total, 5);
    println!("✅ Tier 3 threshold: {}/{}", required, total);

    // Test review periods (using static methods)
    let review_period = ThresholdValidator::get_tier_review_period_static(1);
    assert_eq!(review_period, 7);
    println!("✅ Tier 1 review period: {} days", review_period);

    let review_period = ThresholdValidator::get_tier_review_period_static(3);
...
async fn test_governance_fork_export() -> Result<(), Box<dyn std::error::Error>> {
    ...
    use blvm_commons::fork::export::GovernanceExporter;
